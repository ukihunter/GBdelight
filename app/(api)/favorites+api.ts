...
export async function POST(request: Request) {
  try {
    ...
    if (clerkId) {
      const usersByClerk = await sql`SELECT id FROM users WHERE clerk_id = ${clerkId}`;
      if (usersByClerk.length > 0) userId = usersByClerk[0].id;
    }
    
    // 2. If not found by clerkId, try by email
    if (!userId) {
      const usersByEmail = await sql`SELECT id FROM users WHERE email = ${userEmail}`;
    ...
    if (!userId && clerkId) {
      ...
      try {
        ...
        console.error("Failed to insert user, trying final lookup:", insertError);
        // Last ditch effort: someone might have just inserted it
        const finalCheck = await sql`SELECT id FROM users WHERE clerk_id = ${clerkId} OR email = ${userEmail} LIMIT 1`;
        if (finalCheck.length > 0) userId = finalCheck[0].id;
      }
    }

    if (!userId) {
      return Response.json({ error: "User profile not found. Please log in again." }, { status: 404 });
    }
